...
class BinarySearchTree:
    ...
    def post_order_traversal_values(self) -> list:
        """
        Returns a list of values in post-order traversal: left subtree first,
        right subtree, then the root node, useful when leaf nodes have to be
        removed first
        ...
        """
        values = []
        self._post_order_traversal_values(values, self._root)
        return values

    def _post_order_traversal_values(self, values: list, node: Optional[TreeNode]) -> None:
        if node is None:
            return

        self._post_order_traversal_values(values, node.left)
        self._post_order_traversal_values(values, node.right)
        values.append(node.value)


class TestBinarySearchTreePostOrderTraversal(unittest.TestCase):
    def test_empty_tree_empty_list(self):
        bst = BinarySearchTree()

        values = bst.post_order_traversal_values()
    ...
    def test_left_nodes_only(self):
        ...
        output_values = bst.post_order_traversal_values()
    ...
    def test_complex_example(self):
        ...
        expected_values = [16, 48, 32, 128, 64]
